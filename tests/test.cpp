...
template <typename... Args> void print(std::string fmt, Args... args)
{
    fprintf(stderr, (fmt + '\n').c_str(), args...);
}
...
void test_database()
{
    ...
    pm->db->get_data(
        callback_lambda
        {
            ...
            if (row == 0)
                assert_test(label == NULL &&
                                strcmp(data, test_data.c_str()) == 0,
                            "NULL label and data");
            else if (row == 1)
                assert_test(strcmp(label, test_label.c_str()) == 0 &&
                                strcmp(data, test_data.c_str()) == 0,
                            "label and data");
            row++;
            return 0;
...
test_case cases[] = {{"test_database", test_database},
